...
import { SeoService, LanguageService, UtilService, SystemService, AuthService } from '../../../shared/services';
...
export class ProfileUpdateComponent implements OnInit {
  ...
  public referralCodeCopied: boolean = false;
  ...
  constructor(
    ...
    this.webUrl = this.appService.settings.url;
    ...
    this.userService
      ...
      .then(resp => {
        ...
        if (this.info.notificationSettings === true) {
          this.toasty.success(this.translate.instant('Notification activated successfully!'));
        }
        if (this.info.notificationSettings === false) {
          this.toasty.success(this.translate.instant('Notification deactivated successfully!'));
        }
      })
  ...
  inviteFriend() {
    this.userService.inviteFriend({ email: this.emailInvite }).then(resp => {
      if (resp.data.success) {
        return this.toasty.success(this.translate.instant('Invited Successfully!'));
      }
  ...
  deleteCer(type: string, index: number, certificate = null) {
    if (window.confirm(this.translate.instant('Are you sure to delete this certificate?'))) {
      this.tutorService
  ...
  deleteAvatar() {
    if (this.checkAvatar) {
      if (window.confirm(this.translate.instant('Are you sure to delete your avatar?'))) {
        this.userService
          .deleteAvatar()
          .then(resp => {
  ...
  deleteSubject(item: IMySubject, index: number) {
    if (window.confirm(this.translate.instant('Are you sure to delete this subject?'))) {
