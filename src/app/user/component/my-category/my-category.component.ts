...
export class MyCategoriesComponent implements OnInit {
  ...
  updateCategory(myCategory: IMyCategory, index: number) {
    this.myCategoryService
      .update(
        myCategory._id,
        Object.assign(pick(myCategory, ['originalCategoryId', 'isActive']), {
          isActive: !myCategory.isActive
        })
      )
      .then(resp => {
        if (resp.data) {
          this.myCategories[index] = resp.data;
          this.toasty.success('Updated successfully!');
        }
      })
      .catch(err => {
  ...
  updateSubject(mySubject: IMySubject, index: number) {
    this.mySubjectService
      .update(
        mySubject._id,
        Object.assign(pick(mySubject, ['isActive', 'originalSubjectId']), {
          isActive: !mySubject.isActive,
          myCategoryId: this.selectedCategory._id
        })
      )
      .then(resp => {
        if (resp.data) {
          this.mySubjects[index] = resp.data;
          this.toasty.success('Updated successfully!');
        }
      })
      .catch(err => {
  ...
  updateTopic(myTopic: IMyTopic, index: number) {
    this.myTopicService
      .update(
        myTopic._id,
        Object.assign(pick(myTopic, ['originalTopicId', 'isActive', 'price']), {
          isActive: !myTopic.isActive,
          myCategoryId: this.selectedCategory._id,
