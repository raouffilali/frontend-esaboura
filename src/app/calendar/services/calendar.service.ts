...
export class CalendarService {
  ...
  createRecurring(data): Promise<any> {
    return this.restangular.all('recurring-schedule').post(data).toPromise();
  }

  loadListRecurring(query): Promise<any> {
    return this.restangular.one('recurring-schedule').get(query).toPromise();
  }

  removeRecurring(id): Promise<any> {
    return this.restangular.one('recurring-schedule', id).customDELETE().toPromise();
  }
}
