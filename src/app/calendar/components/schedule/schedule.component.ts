...
export class ScheduleEditComponent implements OnInit {
  ...
  public calendarOptions: CalendarOptions = {
    ...
    eventDidMount: function (info) {
      ...
      if (item.webinarName && item.webinarName.length > 0) {
        ...
        info.el.setAttribute('ngbTooltip', info.event.extendedProps.item.webinarName);
  ...
  dragAllow(dropInfo, event: EventApi) {
    const { isDisabled } = event.extendedProps;
    if (isDisabled) {
      this.toasty.error(this.translate.instant('Cannot update this slot!'));
      return false;
    }
    ...
    if (minute > this.slotDuration) {
      this.toasty.error(this.translate.instant(`Maximum time allowed is ${this.slotDuration} minutes!`));
      return false;
    }
  ...
  eventClick($event: EventClickArg) {
    ...
    if (window.confirm(this.translate.instant('Are you sure to delete this event?'))) {
  ...
  select($event: any) {
    ...
    this.calendar
      ...
      .then(resp => {
        ...
        this.toasty.success(this.translate.instant(message));
        this.onChange.emit(this.isFree);
