...
export class BookingComponent implements OnInit {
  ...
  chooseSlot(time: any) {
    if (!this.isLoggedin) {
      return this.toasty.error(this.translate.instant('Please Log in to book the 1-1 class appointment'));
    }
    this.timeSelected = {
  ...
  bookingAppointment() {
    ...
    modalStripe.componentInstance.rtl = this.rtl;
    ...
    modalStripe.result.then(
      result => {
        if (result.confirmed && result.payMethod) {
          this.booking.payMethod = result.payMethod;
          ...
          } else if ((this.salePrice <= 0 && this.appliedCoupon) || result.payMethod === 'bonus-class') {
            this.appointmentService
              .create(this.booking)
              .then(() => {
                this.toasty.success(this.translate.instant('Booking successfully!'));
    ...
    if (!this.isLoggedin) this.toasty.error(this.translate.instant('Please loggin to use this feature!'));
      ...
      this.tutorFavoriteService
        ...
        .then(res => {
          ...
          this.toasty.success(this.translate.instant('Removed from your favorite tutor list successfully'));
        })
        .catch(() => this.toasty.error(this.translate.instant('Something went wrong, please try again!')));
    }
  }
